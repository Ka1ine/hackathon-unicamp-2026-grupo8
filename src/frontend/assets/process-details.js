(() => {
  ...
  function renderAnalysis() {
    ...
    const aggressiveness = Number.isInteger(policy.aggressiveness)
      ? policy.aggressiveness
      : Number(process.aggressiveness ?? 50);
    $('strategy-aggressiveness').textContent=`Agressividade: ${Math.max(0, Math.min(100, aggressiveness))}%`;
    $('strategy-values').textContent=agreement
      ? `Proposta inicial: ${money(policy.proposed_value_initial || 0)} · Teto: ${money(policy.proposed_value_maximum || 0)}`
