"""Execute: python -m unittest discover -s src/frontend/tests (na raiz)."""
import sys
...
class ProcessosFlowTest(unittest.TestCase):
    def test_configuracao_exibe_e_atualiza_agressividade(self):
        app = AppTest.from_file(str(Path(__file__).resolve().parents[1] / "app.py"))
        app.run(timeout=20)
        app.text_input[0].set_value("demo@enter.com")
        app.text_input[1].set_value("123456")
        app.button[0].click().run()
        next(button for button in app.button if button.label == "Configurações").click().run(timeout=20)

        slider = app.slider(key="_pref_agressividade")
        self.assertEqual(slider.value, 50)
        slider.set_value(80).run(timeout=20)
        self.assertEqual(app.session_state["agressividade"], 80)
