...
from components.preferencias import agressividade, perfil
    ...
    for indice, processo in enumerate(filtrados):
        ...
        with st.container(key=f"process_card_{indice}"):
            ...
            if st.button(
                ...
                politica, erro = atualizar_politica(processo["id"], agressividade())
                if politica:
                    st.session_state[f"politica_personalizada_{processo['id']}"] = politica
                    st.session_state[f"politica_agressividade_{processo['id']}"] = agressividade()
                else:
