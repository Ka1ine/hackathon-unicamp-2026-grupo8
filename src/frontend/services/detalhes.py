...
def carregar_linha_do_tempo(numero, erros):
    ...
    pasta_processo = Path(__file__).resolve().parents[3] / "data" / "example_cases" / str(numero)
...
def status_do_processo(politica):
    """Cria o selo do cabeçalho a partir da recomendação individual do modelo."""
    recomendacao = (politica or {}).get("next_recommended_action")
    if isinstance(recomendacao, str) and recomendacao.strip():
        return f"Recomendação: {recomendacao.strip()}"
...
def carregar_detalhes(numero, politica_personalizada=None, aggressiveness=50):
    ...
    politica = politica_personalizada or carregar_politica(processo["id"], erros)
    return {
        ...
        "status": status_do_processo(politica), "documents": documentos,
