...
class SettlementEngine:
    ...
    def predict(self, df_cases: pd.DataFrame, aggressiveness: float = 50) -> pd.DataFrame:
        """
        aggressiveness: percentual contínuo de 0 (mais conservador) a 100
                        (mais agressivo). 50 representa a política padrão.
        """
        ...
        # A política conservadora aumenta os valores de negociação e reduz os
        # limiares para sugerir acordo; a agressiva produz o efeito inverso.
        conservatism = 1 - aggressiveness / 100
        delta = (conservatism - 0.50) * 0.30
        ...
        threshold_shift = (aggressiveness - 50) / 100 * 0.15
        moderate_threshold = float(np.clip(self.limiar_otimo + threshold_shift, 0.05, 0.90))
        high_threshold = float(np.clip(0.80 + threshold_shift, moderate_threshold + 0.05, 0.98))
        for p in p_loss:
            if p >= high_threshold:
        ...
        output["probabilidade_derrota"] = np.round(p_loss, 4)
